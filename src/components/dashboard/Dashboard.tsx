...
import WhiteNoisePlayer from '@/components/ui/WhiteNoisePlayer'; // Add this import
...
export default function Dashboard({ userProfile: propUserProfile, onStartProfileSetup }: DashboardProps) {
  ...
  // Fetch strengths using useMindfulness hook and destructure fetchEntries
  const { entries: mindfulnessEntries, isLoading: mindfulnessLoading, fetchEntries } = useMindfulness({ type: 'strength', autoFetch: true });
  ...
  fetchEntries(); // Call fetchEntries to refresh the list in Dashboard
  ...
  useEffect(() => {
    if (mindfulnessEntries.length > 1) {
      strengthInterval.current = setInterval(() => {
        setCurrentStrengthIndex(prev => (prev + 1) % mindfulnessEntries.length);
...
<div className="grid grid-cols-2 gap-4">
    ...
    <div className="bg-[#ffe4e6] p-5 rounded-3xl shadow-sm">
        ...
        {isAuthenticated && (
            <>
                {isSupportGiver ? (
                        ...
                        <div className="flex items-center justify-center">
                            <div className="relative w-28 h-28">
                                <svg className="w-28 h-28 transform -rotate-90" viewBox="0 0 100 100">
                                    <circle cx="50" cy="50" r="35" fill="none" stroke="#c7bddb" strokeWidth="10" />
                                    <circle 
                                        cx="50" 
                                        cy="50" 
                                        r="35" 
                    ...
                    >
                       <h2 className="text-xl font-bold text-gray-900">Celebrate</h2>
                       <div className="relative h-24 overflow-hidden bg-white bg-opacity-40 rounded-lg p-3 flex-grow flex items-center justify-center">
                                {/* Use mindfulnessEntries here for displaying strengths */}
                                 {mindfulnessLoading && mindfulnessEntries.length === 0 ? (
                               <div className="text-center text-gray-600 italic">Loading strengths...</div>
                              ) : mindfulnessEntries.length > 0 ?(
                            (mindfulnessEntries as StrengthEntry[]).map((strength, index) => (
                                   <div 
                                       key={(strength.id)} 
                       ...
                       {/* Carousel indicators */}
                       {mindfulnessEntries.length > 1 && (
                           <div className="flex justify-center items-center space-x-1.5">
                               {mindfulnessEntries.map((_, index) => (
                                   <div 
                                       key={index}
        ...
        {!isAuthenticated && (
            <>
                <h2 className="text-xl font-bold text-gray-900 mb-4">Start now</h2>
    ...
    <div className="bg-[#ffe4e6] p-5 rounded-3xl shadow-sm flex flex-col">
        <h2 className="text-xl font-bold text-gray-900 mb-2">InstaCalm</h2>
        ...
        {showFeedbackModal && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl relative">
              <button
                className="absolute top-12 right-5 text-gray-400 hover:text-gray-600"
