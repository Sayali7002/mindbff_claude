...
const personalities = [
  { key: 'default', label: 'Default Friend', desc: 'Empathetic, supportive' },
...
export default function AICompanion() {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        setError("Please try keyboard's voice typing")
      }
  ...
  const handleSendMessage = async (message: string) => {
    ...
    if (isQuickResponse) {
      ...
      setHasUserSentFirstMessage(true); // Set to true after the first message

    }
        // Clear quick responses after the first user message
  ...
  const toggleAnonymousMode = () => {
    if (!isAnonymous && messages.length > 1) {
      ...
      if (window.confirm("Enabling anonymous mode will clear your current chat history. Continue?")) {
        setMessages([{
          role: 'assistant',
          content: "Hello! Nice to meet you. I'm here to listen and support you. How are you feeling today?",
          timestamp: new Date().toISOString()
        }]);
        setIsAnonymous(true);
      }
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      {/* Main chat area with full screen styling */}
       ...
       <div className="flex-1 overflow-y-auto px-2 py-6 pb-20" style={{ minHeight: 0 }}>
