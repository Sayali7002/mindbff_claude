...
export default function EnhancedMeditation({ onClose, currentResource }: EnhancedMeditationProps) {
  ...
  const updateStreak = async (activityType: 'mindfulness') => {
    console.log(`Attempting to update ${activityType} streak for user ${user?.id}`);
    
    // Ensure user is authenticated before attempting API call
    if (!isAuthenticated) {
      console.error('User not authenticated for streak update.');
      // No need to throw here, as it's handled by handleMarkComplete for non-authenticated users
      return; 
    }
    ...
    try {
      ...
      if (sessionError) {
        ...
        throw new Error('Failed to retrieve session for streak update.');
      }
      if (!session) {
        console.error('No session found, cannot update streak.');
      ...
      const response = await fetch('/api/mindfulness/streak', {
        ...
        body: JSON.stringify({ activityType }), // userId is handled by middleware
      });

      if (!response.ok) {
        const errorData = await response.json();
        console.error('API response error:', errorData);
        throw new Error(`Failed to update streak via API: ${errorData.message || response.statusText}`);
      }
      ...
      // but current issue is API call failure due to auth.
    }
  };
